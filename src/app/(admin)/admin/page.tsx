...
import Link from 'next/link'; // 💡 수정 페이지 이동을 위해 추가
...
export default function IntegratedAdminPage() {
  // 💡 2. activeTab 상태에 'posts' 추가
  const [activeTab, setActiveTab] = useState<'analytics' | 'contacts' | 'members' | 'posts'>('analytics');
  const [contacts, setContacts] = useState<Contact[]>([]);
  const [profiles, setProfiles] = useState<Profile[]>([]);
  const [posts, setPosts] = useState<SecurityAlert[]>([]); // 게시글 상태 추가
  ...
  useEffect(() => { setPostPage(1); }, [postSearch, postSort]); // 검색/정렬 변경 시 1페이지로
  ...
  // 💡 4. 게시글 캐싱 및 페이징 (검색 및 정렬 포함)
  const processedPosts = useMemo(() => {
    let result = [...posts];
    if (postSearch.trim()) {
      result = result.filter(p => p.title.toLowerCase().includes(postSearch.toLowerCase()));
    }
    result.sort((a, b) => {
